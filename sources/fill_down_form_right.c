...
/*   Updated: 2016/11/09 16:07:48 by rabougue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		fill_from_down_right(t_env *env)
{
	...
	while (ffdr.piece_y < env->size_form_y)
	{
		...
		{
			if ((env->piece[DRPY][DRPX] == '*' && (env->map[DRPY + DRMY][DRPX +
				DRMX] == env->player_min[0] ||
				env->map[DRPY + DRMY][DRPX + DRMX] == env->player[0])))
				ffdr.me++;
			else if (env->piece[DRPY][DRPX] == '*' &&
				env->map[DRPY + DRMY][DRPX + DRMX] != '.')
				ffdr.ennemi++;
			++ffdr.piece_x;
		}
